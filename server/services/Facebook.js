...
class FB {
  ...
  getFBInfo = (videoUrl, cookie, useragent) => {
    const headers = {
      "sec-fetch-user": "?1",
      "sec-ch-ua-mobile": "?0",
      "sec-fetch-site": "none",
  ...
  try {
    ...
    if (!get.status) {
      ...
      let get2 = await axios.get(`https://krxuv-api.vercel.app/api/snapsave?apikey=Krxuvonly&url=${url}`)
      let hasil = get2.data
      let gett = await Func.getBuffer(hasil.results.data[0].url)
  ...
  async getFacebookHD(bot, chatId, userName) {
    let load = await bot.sendMessage(chatId, 'Loading, please wait.');
    let db = await Databs.readDb('/tmp/database.json');
    try {
  ...
  async getFacebookAudio(bot, chatId, userName) {
    let load = await bot.sendMessage(chatId, 'Loading, please wait.');
    let db = await Databs.readDb('/tmp/database.json');
    try {
      let buff = await Func.getBuffer(db[chatId].fbmp3)
      await fs.writeFileSync('/tmp/Facebook_audio_' + chatId + '.mp3', buff);
      await bot.sendAudio(chatId, '/tmp/Facebook_audio_' + chatId + '.mp3', { caption: `Bot by @Krxuvv` });
      await bot.deleteMessage(chatId, load.message_id);
      db[chatId] = {
        fbnormal: '',
        fbhd: '',
